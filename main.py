...
required_env_vars = ['OPENAI_API_KEY', 'MONGODB_CONNECTION_STRING']
...
if page == "Home":
    ...
    def get_suitable_profession(query):
        try:
            ...
            st.error(f"Error in getting suitable profession: {str(e)}")
            return None
    ...
    def store_question_in_collection(profession, question):
        try:
            ...
            st.error(f"Error storing question in collection: {str(e)}")
    ...
    def store_in_unknown_agents(profession, question):
        try:
            ...
            st.error(f"Error storing in unknown agents: {str(e)}")
    ...
    if st.button("Find Suitable Professional"):
        if user_input.strip():
            ...
            if suggested_profession:
                ...
                st.error("Failed to get a suitable profession. Please try again.")
        else:
    ...
    try:
        ...
        st.error(f"Error fetching unknown agents data: {str(e)}")
    ...
    try:
        ...
        st.error(f"Error fetching user profiles: {str(e)}")